...
export default function AdminLayoutClient({ children, initialRole }) {
  ...
  // The role is resolved server-side (see app/admin/layout.jsx) and passed in
  // as `initialRole`, so the correct nav renders on the very first paint —
  // no client-side fetch, no flash of the wrong role's nav items while a
  // `null`/default state would otherwise be visible.
  ...
  function handleSignOut() {
    ...
    // A hard navigation (not router.push) so sign-out doesn't wait on a
    // client-side RSC transition — that fetch is itself gated behind
    // middleware's Supabase session check, which made the "Signing out…"
    // button sit for a visible beat before /login appeared.

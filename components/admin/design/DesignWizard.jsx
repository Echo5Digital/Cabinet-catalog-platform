          ...
          <div key={i} className="flex items-center shrink-0">
            ...
            <div className="flex flex-col items-center">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                    ...
                    : active
                    ? "text-white !text-white ring-4 ring-offset-2"
                ...
                style={
                    ...
                    ? { background: "#4F46E5" }
                    : active
                    ? { background: "#4F46E5", ringColor: "rgba(79,70,229,0.2)" }
                    : {}
                }
              >
                {done ? (
              ...
              <span
                className={`mt-1.5 text-[10px] font-semibold uppercase tracking-wide whitespace-nowrap ${
                  active ? "text-[#4F46E5]" : done ? "text-stone-400" : "text-stone-300"
                }`}
              >
                {step.label}
              </span>
            ...
            {i < STEPS.length - 1 && (
              <div
                ...
                style={{ background: i < current ? "#4F46E5" : "#e7e5e4" }}
              />
            )}
          </div>
        );
  ...
  return (
    <div className="min-h-full bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Page header */}
        <div className="mb-8">
          <p className="text-xs font-semibold text-[#4F46E5]/60 uppercase tracking-widest mb-1">
            Admin · Design Studio
          </p>
          <h1
      ...
      {notify && (
        <div
          className={`fixed bottom-5 right-5 z-50 flex items-center gap-3 px-4 py-3 rounded-xl shadow-xl text-sm font-semibold max-w-xs border ${
            notify.type === "success"
              ? "bg-white text-[#111827] border-[rgba(79,70,229,0.2)]"
              : "bg-white text-red-700 border-red-200"
          }`}
        >
          {notify.type === "success" ? (
            <span className="w-6 h-6 rounded-full flex items-center justify-center shrink-0" style={{ background: "#4F46E5" }}>

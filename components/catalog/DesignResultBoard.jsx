...
const SUGGESTION_FIELD_LABELS = {
  upper_color: "Upper Cabinet Color",
  lower_color: "Lower Cabinet Color",
...
export default function DesignResultBoard({
  ...
  // The iframe sends VIEWER_READY immediately after its inline script runs
  // (pure WebGL, no CDN — fires < 1 ms after page load).
  // We respond with LOAD_IMAGE. A retry covers the rare case where
  // VIEWER_READY fires before this effect registers its window listener.
  useEffect(() => {
    ...
    function onMessage(e) {
      if (e.data?.type === "VIEWER_READY") send();
    }
    window.addEventListener("message", onMessage);

    // Strategy B: also try immediately + short-delay fallback
    send();
    ...
    <div className="rounded-2xl overflow-hidden border border-stone-200 shadow-sm bg-white">
      ...
      {lightboxOpen && image_url && (
        <div
          ...
          style={{ background: threeDImage ? "radial-gradient(ellipse at center, #0d0d1a 0%, #000 100%)" : "rgba(0,0,0,0.92)" }}
          onClick={closeLightbox}
        >
          {/* ── 360° AR Viewer (Three.js iframe) / static image (2D) ──────── */}
          {threeDImage ? (
            /* ── Three.js WebGL viewer — real orbit/zoom/AR, not CSS tricks ── */
            <div
              className="relative rounded-2xl overflow-hidden"
              style={{
                width: "92vw",
                height: "82vh",
                boxShadow: "0 0 100px rgba(99,102,241,0.30), 0 0 0 1px rgba(99,102,241,0.20)",
                background: "#050508",
              }}
            ...
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              ...
              {threeDLoading && (
                <div className="absolute inset-0 rounded-xl z-10 flex flex-col items-center justify-center gap-4 overflow-hidden">
                  ...
                  <div className="absolute inset-0 rounded-xl animate-pulse pointer-events-none" style={{ background: "linear-gradient(110deg,transparent 35%,rgba(99,102,241,0.08) 50%,transparent 65%)" }} />
                  <div className="relative z-10 flex flex-col items-center gap-3">
                    <div className="relative w-11 h-11">
                      ...
                      <div className="absolute inset-2 rounded-full border border-indigo-300/20 animate-ping" />
                    </div>
                    <p className="text-white text-[11px] font-semibold tracking-widest uppercase animate-pulse">Rendering Immersive View</p>
                    <p className="text-white/40 text-[10px]">Hang tight, this takes a moment…</p>
                  </div>
                </div>
              )}
            </div>
          ...
          >
            {threeDImage ? (
              <button
                ...
                className="px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 text-white text-xs font-semibold transition"
              >
                ← Back to 2D
              </button>
            ) : (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white text-xs font-semibold transition"
              >
                {threeDLoading ? (
                  <>
                  ...
                  <>
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M15 10l4.553-2.07A1 1 0 0121 8.87V15.13a1 1 0 01-1.447.9L15 14M3 8.5A1.5 1.5 0 014.5 7h8A1.5 1.5 0 0114 8.5v7A1.5 1.5 0 0112.5 17h-8A1.5 1.5 0 013 15.5v-7z"/>
                    </svg>
                    View Immersive
                  </>
                )}
            ...
            {threeDError && (
              <span className="text-red-400 text-xs">{threeDError}</span>
            )}
          </div>

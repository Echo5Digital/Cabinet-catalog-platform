...
export default function AiPreviewPanel({ onRegenerate, onClose, primaryColor = "#1C1917" }) {
  ...
  return (
    <>
      ...
      />
      <style>{`
        .ai-regen-btn { border-color: ${primaryColor}40; color: #57534e; }
        .ai-regen-btn:hover { border-color: ${primaryColor}; color: ${primaryColor}; background-color: ${primaryColor}08; }
        .ai-dl-btn { border-color: #e7e5e4; color: #57534e; }
        .ai-dl-btn:hover { border-color: ${primaryColor}55; color: ${primaryColor}; background-color: ${primaryColor}06; }
      `}</style>
        ...
        {aiLoading && (
          <div className="space-y-3">
            <div className="w-full aspect-video rounded-xl bg-stone-100 animate-pulse flex flex-col items-center justify-center gap-3">
              <svg className="w-8 h-8 animate-spin" fill="none" viewBox="0 0 24 24" style={{ color: primaryColor }}>
          ...
          <>
            ...
            {!aiImageUrl.startsWith("data:") && (
              <a
                ...
                className="flex items-center justify-center gap-1.5 w-full py-2 rounded-xl border text-xs font-medium transition ai-dl-btn"
              >
      ...
      {!aiLoading && (aiImageUrl || aiError) && (
        <div className="px-4 py-3 border-t border-stone-100">
          <button
            onClick={onRegenerate}
            disabled={aiLoading}
            className="w-full flex items-center justify-center gap-2 py-2 rounded-xl border text-xs font-semibold transition ai-regen-btn"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>

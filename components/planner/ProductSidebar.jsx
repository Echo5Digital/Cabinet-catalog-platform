...
export default function ProductSidebar({ products = [], isOpen, onToggle }) {
  ...
  // Default: first present category (Base Cabinets) is open
  const firstCat = presentCategories[0] || null;
  const [openCategories, setOpenCategories] = useState(() => new Set(firstCat ? [firstCat] : []));
...
function SidebarContent({ products, grouped, presentCategories, openCategories, toggleCategory }) {
  return (

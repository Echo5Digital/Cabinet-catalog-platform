...
VANITY: a TIGHT, close-up product shot of ONLY the vanity wall (cabinet, countertop, sink, faucet,
mirror) filling 80%+ of the frame. NO toilet, NO shower, and NO tub anywhere in the frame, even
partially. This is NOT a wide bathroom shot — image models default to a room-scale composition
unless explicitly constrained, so the tight crop must be actively enforced.
SHOWER AREA: a TIGHT, close-up shot of ONLY the shower enclosure filling 80%+ of the frame — NO
bathtub unless the selected style is "Tub & Shower Combo" or a tub was explicitly requested. NO
vanity and NO toilet anywhere in the frame, even partially. Like Vanity, this is a single-fixture
close-up, not a wide bathroom shot — actively enforce the tight crop, don't leave it to chance.
...
VANITY FAUCET: must use ONLY the selected vanity faucet style — its NAME (e.g. "Gooseneck",
"Widespread", "Vessel-Height") is not decorative, it specifies an exact silhouette (see the
faucet description provided in CUSTOMER INPUT below). A generic single-handle faucet substituted
for a different named style has FAILED this rule just as surely as using the wrong color would.
SHOWER FAUCET/FIXTURE: must use ONLY the selected shower faucet style, when a shower enclosure is
present — same rule: the name specifies an exact fixture silhouette (round vs. square head, rain
vs. arm-mounted, exposed valve + handheld vs. concealed valve + overhead), not a generic showerhead.
...
floor-standing vanity, framed vs. frameless glass, and each faucet's exact shape as described above)
is LOCKED exactly like color — it is not a suggestion and must not be softened, merged, or substituted
for a more "generic" or "safe" configuration.

...
/** Shared helper: get an OpenAI client for admin one-shot calls. */
async function getAdminClient(tenantId) {
  const { apiKey } = await getAIConfig(tenantId);
  return new OpenAI({ apiKey });
}

export async function summarizeLead(leadData, tenantId) {
...
export async function explainVersionDiff(diff, tenantId) {
  const client = await getAdminClient(tenantId);
...
export async function draftProductDescription(productData, tenantId) {
  const client = await getAdminClient(tenantId);

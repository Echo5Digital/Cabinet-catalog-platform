...
STEP 5 — Apply User Style Inputs (finish, hardware, hood style, countertop, flooring, special requests/notes)
...
Sentence 1: Describe cabinet door style, hardware finish, and hood style ONLY — be specific (e.g. "shaker doors with matte black bar pulls and a stainless steel chimney hood").
...
IMPORTANT: Hood style is a REQUIRED element of the dalle_prompt. If a hood style was specified, it MUST appear in Sentence 1. If special requests or notes were provided, incorporate any visually relevant aspects into the dalle_prompt sentences.
...
1. Compare the product's width against the cabinet_plan estimated_width for that placement zone.
2. Verify standard cabinet depth (base: ~24"D, wall: ~12"D) fits the available wall space and countertop overhang.
3. Reject any SKU whose width significantly over- or under-fills its placement zone (more than ±3 inches off the planned run).
4. Prefer products whose dimensions combine efficiently to fill a wall run without excessive filler strips.
5. If room dimensions are provided, verify that total cabinet widths per wall do not exceed available wall length minus standard clearances (doorways, windows, appliance spaces).
dimension_fit field MUST describe in one sentence why this product's dimensions suit its placement zone. Example: "At 36"W it fills the base run beside the range without requiring filler strips."
...
The customer's existing kitchen is shown. ONLY cabinets, countertop, and flooring change.
1. Preserve EXACT room geometry: ceiling height, window positions, walls, door placements.
2. Preserve EXACT surface finishes: wall paint colour, backsplash tiles, ceiling colour.
3. Preserve EXACT fixtures: all light fittings and colour temperature, sink, faucet, and all appliance finishes (unless customer explicitly selected a new appliance colour).
4. Replace cabinets, countertop, and flooring with the customer's selected options ONLY.
5. Analyze the photo for cabinet count and room proportions to inform SKU selection.
6. dalle_prompt Sentence 1: describe new cabinet door style and hardware ONLY.
...
CRITICAL: ONLY the kitchen cabinets (and hood/hardware if specified) change. Every other element must be identical to the source photo.
1. Identify the EXISTING countertop — concept.countertop MUST describe what is visible in the photo (NOT the form selection).
2. Identify the EXISTING flooring — concept.flooring MUST describe what is visible in the photo (NOT the form selection).
3. concept.upper_color and concept.lower_color = customer's selected new cabinet colours.
4. Wall paint, wall colour, and wall surfaces — IDENTICAL to source photo. Do not alter.
5. Backsplash tiles — IDENTICAL to source photo (material, colour, pattern). Do not alter.
6. Lighting — IDENTICAL colour temperature and fixture appearance. Do not alter.
7. Appliances (refrigerator, range, dishwasher) — IDENTICAL finish and position. Do not alter.
8. Windows, doors, door openings, walls, ceiling, room geometry — IDENTICAL to source photo.
9. dalle_prompt Sentence 1: describe the new cabinet door style and hardware ONLY.
10. dalle_prompt Sentence 2: describe soft natural lighting — match existing light quality, do not change it.`;

const ADDENDUM_COUNTERTOP_ONLY = `

--- EXISTING KITCHEN PHOTO — COUNTERTOP ONLY ---
CRITICAL: ONLY the countertop surface changes. Every other element must be identical to the source photo.
1. Identify the EXISTING cabinet colours and door style — concept.upper_color and concept.lower_color MUST describe what is visible in the photo.
2. Identify the EXISTING flooring — concept.flooring MUST describe what is visible in the photo.
3. concept.countertop = customer's selected new countertop material.
4. Wall paint, wall colour, and wall surfaces — IDENTICAL to source photo. Do not alter.
5. Backsplash tiles — IDENTICAL to source photo (material, colour, pattern). Do not alter.
6. Lighting — IDENTICAL colour temperature and fixture appearance. Do not alter.
7. Appliances (refrigerator, range, dishwasher) — IDENTICAL finish and position. Do not alter.
8. Windows, doors, door openings, walls, ceiling, room geometry — IDENTICAL to source photo.
9. dalle_prompt Sentence 1: describe the new countertop material and edge profile ONLY.
10. dalle_prompt Sentence 2: describe soft natural lighting — match existing light quality, do not change it.`;
...
export function buildKitchenDesignPrompt(fields, catalogContext, includeImageAnalysis = false, hasReferenceImages = false) {
  ...
  // Build optional room specifications block — only include lines that were actually provided.
  const roomSpecLines = [
    room_width            ? `Kitchen Width: ${room_width} ft`                      : null,
    room_length           ? `Kitchen Length: ${room_length} ft`                    : null,
    ceiling_height        ? `Ceiling Height: ${ceiling_height} ft`                 : null,
    window_positions      ? `Window Positions: ${window_positions}`                : null,
    door_positions        ? `Door Positions: ${door_positions}`                    : null,
    refrigerator_position ? `Refrigerator Position: ${refrigerator_position}`      : null,
    sink_position         ? `Sink Position: ${sink_position}`                      : null,
    special_features      ? `Special Features / Constraints: ${special_features}`  : null,
  ].filter(Boolean);
  const roomSpecs = roomSpecLines.length > 0
    ? roomSpecLines.join("\n")
    : "Not provided — use standard residential kitchen proportions.";

  // Only mention reference images that are actually being attached to this message.
  const referenceImagesNote = hasReferenceImages
    ? `==================================================\nREFERENCE IMAGES PROVIDED\nOne or more reference images are attached to this message. Each is labeled with its purpose (e.g. layout reference, color swatches, customer photo). Visually analyze every attached image before generating output — they are mandatory truth references.`
    : "";

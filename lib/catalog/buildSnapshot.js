...
export async function buildCatalogSnapshot(lineId, tenantId) {
  ...
  await backfillFinishIds(lineId, tenantId, admin);
  ...
  // Check each active product has ≥1 available finish.
  // If no mappings exist at all, all line finishes are implicitly available — not a blocker.
  // Only block if explicit mappings exist but none are marked is_available.
  for (const product of products ?? []) {
    const mappings = pfByProduct[product.id] ?? [];
    if (mappings.length > 0) {
      const availableFinishes = mappings.filter((pf) => pf.is_available);
  ...
  const snapshot = {
    ...
    products: (products ?? []).map((p) => ({
      ...
      finish_ids: (pfByProduct[p.id] ?? []).length === 0
        ? (finishes ?? []).map((f) => f.id)   // no explicit mappings → all line finishes available
        : (pfByProduct[p.id] ?? [])

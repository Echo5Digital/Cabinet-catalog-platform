...
export async function sendDesignQuoteEmail({
  ...
  const safeOriginalUrl = safeImgUrl(dp.imageUrl); // original customer photo
  ...
  // ── Kitchen render / Before-After ────────────────────────────────────────
  const renderHtml = isRemodel && safeOriginalUrl && safeDesignImageUrl ? `
    <div style="margin:20px 0">
      <p style="font-size:11px;font-weight:700;color:#6E1020;letter-spacing:.09em;text-transform:uppercase;margin:0 0 10px">Before &amp; After</p>
      <table style="width:100%;max-width:560px;border-collapse:collapse">
        <tr>
          <td style="width:50%;padding-right:6px;vertical-align:top">
            <img src="${safeOriginalUrl}" alt="Before" style="width:100%;border-radius:8px;border:1px solid #e7e5e4;display:block">
            <p style="margin:5px 0 0;font-size:11px;font-weight:600;color:#78716c;text-align:center">Before</p>
          </td>
          <td style="width:50%;padding-left:6px;vertical-align:top">
            <img src="${safeDesignImageUrl}" alt="After" style="width:100%;border-radius:8px;border:1px solid #e7e5e4;display:block">
            <p style="margin:5px 0 0;font-size:11px;font-weight:600;color:#4F46E5;text-align:center">AI Generated</p>
          </td>
        </tr>
      </table>
    </div>` : safeDesignImageUrl ? `
    <div style="margin:20px 0">
      ...
      <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1e293b">
        ...
        <div style="padding:28px 24px;border:1px solid #e7e5e4;border-top:none;border-radius:0 0 8px 8px">
          ...
          <p style="color:#64748b;margin:0 0 6px;line-height:1.7;font-size:13px">
            ${isRemodel
              ? "Your kitchen remodel design concept is ready. The before &amp; after comparison and full design document are attached to this email."
...
export async function sendStaffQuoteNotification({
  ...
  const from = `"Cabinets & Remodeling Depot" <${process.env.SMTP_FROM || process.env.SMTP_USER}>`;
  const to   = "sales@cabinetsandremodelingdepot.com";

  const productRows = Array.isArray(products) ? products.filter(Boolean) : [];

  const productsHtml = productRows.length > 0 ? `
    <div style="margin:20px 0">
      <p style="font-size:11px;font-weight:700;color:#6E1020;letter-spacing:.09em;text-transform:uppercase;margin:0 0 8px">Selected Products</p>
      <table style="width:100%;max-width:560px;border-collapse:collapse;font-size:12px">
        <thead>
          <tr style="background:#1C1917;color:#fff">
            <th style="padding:9px 12px;text-align:left;font-weight:600;font-size:11px">SKU</th>
            <th style="padding:9px 12px;text-align:left;font-weight:600;font-size:11px">Name</th>
            <th style="padding:9px 12px;text-align:left;font-weight:600;font-size:11px">Type</th>
            <th style="padding:9px 12px;text-align:left;font-weight:600;font-size:11px">Dimensions</th>
          </tr>
        </thead>
        <tbody>
          ${productRows.map((p, i) => `
            <tr style="background:${i % 2 === 0 ? "#fff" : "#F8F6F3"};border-bottom:1px solid #e7e5e4">
              <td style="padding:8px 12px;color:#1c1917">${esc(p.sku)}</td>
              <td style="padding:8px 12px;color:#1c1917">${esc(p.name)}</td>
              <td style="padding:8px 12px;color:#57534e">${esc(p.type)}</td>
              <td style="padding:8px 12px;color:#57534e">${esc(p.dimensions)}</td>
            </tr>`).join("")}
        </tbody>
      </table>
    </div>` : "";

  const plainProducts = productRows.length > 0
    ? "\nSELECTED PRODUCTS:\n" + productRows.map((p) => `  ${p.sku || ""} | ${p.name || ""} | ${p.type || ""} | ${p.dimensions || ""}`).join("\n")
    : "";

  const plainText = [
    "New quote request from the AI Kitchen Designer.",
    "",
    "CUSTOMER DETAILS:",
    `  Name: ${name}`,
    `  Email: ${email}`,
  ...
  await transporter.sendMail({
    ...
    to,
    replyTo: email,
    subject: "New Quote Request - Cabinet Catalog",
    text: plainText,
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1e293b">
        ...
        <!-- Header -->
        <div style="background:#6E1020;padding:28px 24px;border-radius:8px 8px 0 0">
          <p style="margin:0;font-size:10px;font-weight:600;color:rgba(255,255,255,0.55);letter-spacing:.12em;text-transform:uppercase">New Quote Request</p>
          <p style="margin:8px 0 0;font-size:22px;font-weight:700;color:#fff;letter-spacing:-.3px">AI Kitchen Designer</p>
        </div>

        <!-- Body -->
        <div style="padding:28px 24px;border:1px solid #e7e5e4;border-top:none;border-radius:0 0 8px 8px">

          <div style="background:#F8F6F3;border-radius:8px;padding:18px 20px;margin:0 0 20px">
            <p style="font-size:11px;font-weight:700;color:#6E1020;letter-spacing:.09em;text-transform:uppercase;margin:0 0 12px">Customer Details</p>
            <p style="margin:0 0 7px;font-size:13px;color:#1c1917"><span style="color:#78716c;font-size:12px">&#8226; Name:&nbsp;</span>${esc(name)}</p>
            <p style="margin:0 0 7px;font-size:13px;color:#1c1917"><span style="color:#78716c;font-size:12px">&#8226; Email:&nbsp;</span>${esc(email)}</p>
            ${phone ? `<p style="margin:0 0 7px;font-size:13px;color:#1c1917"><span style="color:#78716c;font-size:12px">&#8226; Phone:&nbsp;</span>${esc(phone)}</p>` : ""}
            ${address ? `<p style="margin:0;font-size:13px;color:#1c1917"><span style="color:#78716c;font-size:12px">&#8226; Address:&nbsp;</span>${esc(address)}</p>` : ""}
          </div>

          ${projectDescription ? `
          <div style="margin:0 0 20px">
            <p style="font-size:11px;font-weight:700;color:#6E1020;letter-spacing:.09em;text-transform:uppercase;margin:0 0 8px">Project Details</p>
            <p style="margin:0;font-size:13px;color:#1c1917;line-height:1.6;white-space:pre-line">${esc(projectDescription)}</p>
          </div>` : ""}

          ${notes ? `
          <div style="margin:0 0 20px">
            <p style="font-size:11px;font-weight:700;color:#6E1020;letter-spacing:.09em;text-transform:uppercase;margin:0 0 8px">Notes</p>
            <p style="margin:0;font-size:13px;color:#1c1917;line-height:1.6;white-space:pre-line">${esc(notes)}</p>
          </div>` : ""}
          ...
          ${beforePhotoUrl ? `
          <div style="margin:20px 0">
            <p style="font-size:11px;font-weight:700;color:#6E1020;letter-spacing:.09em;text-transform:uppercase;margin:0 0 10px">Before Photo</p>
            <img src="${esc(beforePhotoUrl)}" alt="Before" style="width:100%;max-width:400px;border-radius:8px;border:1px solid #e7e5e4;display:block">
          </div>` : ""}

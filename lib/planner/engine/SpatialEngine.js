...
import { snapToGrid } from "@/lib/planner/snap";
import {
  CABINET_HEIGHTS_FT,
...
function cmdPlaceItem(scene, { id, product, position }, room) {
  const snappedX = snapToGrid(position?.xFt ?? 0, 1);
  const snappedZ = snapToGrid(position?.zFt ?? 0, 1);
...
function cmdMoveItem(scene, { id, position }, room) {
  return {
    ...
    items: scene.items.map((item) => {
      ...
      const snappedX = snapToGrid(position.xFt, 1);
      const snappedZ = snapToGrid(position.zFt, 1);

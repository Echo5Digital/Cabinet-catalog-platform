...
export function generateLayout(layout, roomDimensions, products) {
  ...
  // Pre-compute fixture footprint exclusions per zone so that fill cursors
  // jump over fixture slots cleanly — no partial-overlap deletions, no gaps.
  // Refrigerator exclusions are also applied to the corresponding upper zone
  // so wall cabinets never overlap the tall fridge column.
  const slots = getFixtureSlots(layout, W, L);
  const exclusionsByZone = {};  // zoneId → [{lo, hi}] in run-direction coords
  // No automatic clearance gap: cabinets fill right up to the cooktop/range.
  const RANGE_CLEARANCE_FT = 0;
  for (const [slotKey, slot] of Object.entries(slots)) {
    ...
    const isRange = slotKey === "range";
    const isFridge = slotKey === "fridge";
    if (slotNS) {
      // NS zone (west/east wall): run axis is Z.
      // Range clearance applies along Z for NS ranges.
      const lo = isRange ? slot.z - RANGE_CLEARANCE_FT : slot.z;
      const hi = isRange ? slot.z + slot.d + RANGE_CLEARANCE_FT : slot.z + slot.d;
      exclusionsByZone[slot.zoneId].push({ lo, hi });
      // Mirror fridge exclusion to the upper zone directly above it so wall
      // cabinets don't overlap the tall refrigerator column.
      if (isFridge) {
        const upperZoneId = slot.zoneId.replace("base-", "upper-");
        if (!exclusionsByZone[upperZoneId]) exclusionsByZone[upperZoneId] = [];
        exclusionsByZone[upperZoneId].push({ lo, hi });
      }
    } else {
      // EW zone (north wall): run axis is X.
      // Range clearance applies along X for EW ranges.
      const lo = isRange ? slot.x - RANGE_CLEARANCE_FT : slot.x;
      const hi = isRange ? slot.x + slot.w + RANGE_CLEARANCE_FT : slot.x + slot.w;
      exclusionsByZone[slot.zoneId].push({ lo, hi });
      // Mirror fridge exclusion to the upper zone so wall cabinets don't
      // overlap the tall refrigerator column.
      if (isFridge) {
        const upperZoneId = slot.zoneId.replace("base-", "upper-");
        if (!exclusionsByZone[upperZoneId]) exclusionsByZone[upperZoneId] = [];
        exclusionsByZone[upperZoneId].push({ lo, hi });
  ...
  for (const zone of zones) {
    ...
    if (!zoneExclusions || (zone.type !== "base" && zone.type !== "upper")) {
      // Island / peninsula zones have no fixture exclusions — fill whole zone
    ...
    // Base and upper zones: fill only the segments between (and beyond) fixture footprints
    const ns       = isNS(zone);
    const runStart = ns ? zone.y2d : zone.x2d;
    const runEnd   = runStart + (ns ? zone.depthFt : zone.widthFt);

    // Clamp each exclusion to the zone's actual run extent, then merge
    // overlapping/adjacent exclusions so the fill loop never skips segments
    // due to a lo < runStart (e.g. negative after range clearance expansion).
    const clamped = zoneExclusions.map(({ lo, hi }) => ({
      lo: Math.max(lo, runStart),
      hi: Math.min(hi, runEnd),
    })).filter(({ lo, hi }) => hi > lo + 0.02); // drop zero-width exclusions

    // Merge overlapping exclusions: sort by lo, then merge intervals
    const merged = [];
    for (const ex of clamped.sort((a, b) => a.lo - b.lo)) {
      if (merged.length > 0 && ex.lo <= merged[merged.length - 1].hi + 0.02) {
        merged[merged.length - 1].hi = Math.max(merged[merged.length - 1].hi, ex.hi);
      } else {
        merged.push({ ...ex });
      }
    }
...
/**
 * Returns {sink, range, fridge} placement slots for the given layout and room size.
 *
 ...
 *   - Always placed at the end of a wall run for easy access and natural look.
 *   - L/U/G-Shape: east wall end (far corner from range which is on west wall).
 *   - Parallel: end of east wall run.
 *   - Straight/Island: far right end of north wall.
 */
function getFixtureSlots(layout, W, L) {
  ...
  const FW      = 2.5;   // fridge footprint along its wall (standard 30" wide)
  const FD      = 2.0;   // fridge depth (standard 24" + door swing allowance)
  ...
  switch (layout) {
    ...
    case "Parallel":
      return {
        ...
        // Fridge at the end (south end) of east wall — easy access from both sides
        fridge: { x: W - D, z: cl(L - FW, 0, L - FW), w: FD, d: FW, zoneId: "base-east" },
      };

    // ── Sink on north wall (base-north starts at x=D), Range on west wall ────
    // Fridge goes on east wall at south end — separated from range (west wall)
    case "L-Shape":
    case "U-Shape":
    case "G-Shape":
      return {
        sink:   { x: cl(W/2 - SW/2, D, W - SW), z: 0,                            w: SW, d: D,  zoneId: "base-north" },
        range:  { x: 0,                          z: cl(L*0.40 - RW/2, 0, L - RW), w: D,  d: RW, zoneId: "base-west"  },
        // Fridge on east wall at south end — opposite corner from range
        fridge: { x: W - D,                      z: cl(L - FW, 0, L - FW),        w: FD, d: FW, zoneId: "base-east"  },
      };

    // ── Both on north wall with enforced gap ──────────────────────────────────
    // Fridge at far right end of north wall, fixtures spaced accordingly
    case "Island":
    case "Straight":
    default: {
      const fridgeX = cl(W - FW, D, W - FW);
      let sinkX  = cl(W*0.25 - SW/2, D, fridgeX - SW - MIN_GAP);
      let rangeX = cl(W*0.55 - RW/2, D, fridgeX - RW - 0.5);
      if (rangeX < sinkX + SW + MIN_GAP) rangeX = Math.min(sinkX + SW + MIN_GAP, fridgeX - RW - 0.5);
      return {
        sink:   { x: sinkX,  z: 0, w: SW, d: D, zoneId: "base-north" },
        range:  { x: rangeX, z: 0, w: RW, d: D, zoneId: "base-north" },
        fridge: { x: fridgeX, z: 0, w: FW, d: D, zoneId: "base-north" },
      };
    }
  }
}

/** Builds the three hardcoded fixture scene items (Sink + Range + Refrigerator). */
function buildFixtureItems(layout, W, L, timestamp) {
  const H      = 3.0;  // matches CABINET_HEIGHTS_FT["Sink"/"Range"] in sceneBuilder.js
  const FRIDGE_H = 6.0; // standard refrigerator height ~72" (6 ft)
  const slots  = getFixtureSlots(layout, W, L);
  return [
    {
      id: `fixture-sink-${timestamp}`,  productId: "fixture-sink-dw",
      sku: "FIXTURE-SINK-DW",  name: "Sink + Dishwasher",  category: "Sink",
    ...
    {
      id: `fixture-fridge-${timestamp}`, productId: "fixture-fridge",
      sku: "FIXTURE-FRIDGE", name: "Refrigerator", category: "Refrigerator",
      dimensions: { widthFt: slots.fridge.w, depthFt: slots.fridge.d, heightFt: FRIDGE_H },
      position:   { xFt: slots.fridge.x, yFt: 0, zFt: slots.fridge.z },
      rotation: { yDeg: 0 }, zoneId: slots.fridge.zoneId, wallId: null,
      gltfUrl: null, imageUrl: null, doorCount: null, drawerCount: null, isFiller: false,
      material: { color: "#d6d0ca", roughness: 0.20, metalness: 0.30, finishId: null },
    },
  ];
}

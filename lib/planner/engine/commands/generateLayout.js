...
export function generateLayout(layout, roomDimensions, products) {
  ...
  // Range requires 2 ft of clear counter on each side (NKBA safety clearance).
  // Expand the range exclusion zone by this amount so the zone filler leaves
  // a clear 2 ft gap on both sides — preventing the design-rule warning from
  // firing immediately after procedural generation.
...
function getFixtureSlots(layout, W, L) {
  ...
  switch (layout) {
    ...
    // base-north now starts at x=D (corner owned by base-west), so sink x
    // must be clamped to [D, W-SW] to stay within the trimmed north zone.
    // base-west now starts at z=0, so range z clamp starts at 0.
    case "L-Shape":
    case "U-Shape":
    case "G-Shape":
      return {
    ...
    // base-north starts at x=D, so both fixtures must stay within [D, W-SW].
    case "Island":
    case "Straight":
    default: {
      let sinkX  = cl(W*0.25 - SW/2, D, W - SW);
      let rangeX = cl(W*0.62 - RW/2, D, W - RW);

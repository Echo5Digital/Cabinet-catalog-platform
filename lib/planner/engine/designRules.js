...
export function validateKitchenLayout(items, roomDimensions) {
  ...
  for (const range of ranges) {
    const { xFt, zFt } = range.position;
    const { widthFt, depthFt } = range.dimensions;
    const leftEdge  = xFt;
    const rightEdge = xFt + widthFt;

    // Check cabinets within the clearance zone on each side (same Z band)
    const tooClose = items.filter((i) => {
      ...
      const leftConflict  = ib.maxX > leftEdge  - RANGE_CLEARANCE_FT && ib.maxX <= leftEdge;
      const rightConflict = ib.minX < rightEdge + RANGE_CLEARANCE_FT && ib.minX >= rightEdge;
  ...
  for (const fridge of fridges) {
    ...
    if (clearanceEnd > roomDimensions.length) {
      results.push({
        id: nextId(),
        severity: "warning",

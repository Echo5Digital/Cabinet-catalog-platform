...
export const CABINET_HEIGHTS_FT = {
  ...
  "Wall Cabinets": 2.5,    // 30 in (standard upper)
  "Tall Units":    7.0,    // 84 in
  "Appliances":    3.0,    // default; use product height_in / 12 when available
  "Sink":          3.0,    // 36 in — counter height
  "Range":         3.0,    // 36 in — standard range height
  "Refrigerator":  6.0,    // 72 in — standard French-door refrigerator
  default:         3.0,
};
...
export const CATEGORY_COLORS_3D = {
  ...
  "Sink":          "#bae6fd",
  "Range":         "#fed7aa",
  "Refrigerator":  "#dbeafe",   // cool blue-tinted stainless
  default:         "#e7e5e4",
};
...
export function buildWallsFromLayout(layout, { width: W, length: L }) {
  ...
  // Three perimeter walls — south wall is intentionally omitted so the 3D
  // camera (positioned outside the open south face) can see into the room.
  // This is the standard "open-front / dollhouse" view used by kitchen planners.
  const perimeter = [
  ...
  // Layout-specific extra walls (peninsula is an open counter, not a wall)
  switch (layout) {
    case "G-Shape":
      return [...perimeter];
    default:
...
export function placedItemTo3D(item) {
  ...
  return {
    ...
    // Zone — preserved so Scene3DFixture can determine facing direction
...
export function buildSceneGraph(layout, roomDimensions, placedItems, tenantId = null) {
  ...
  return {
    ...
    camera: {
      mode:     "perspective",
      position: [W / 2, H * 0.85, L + H * 0.5],
      target:   [W / 2, H * 0.15, L * 0.35],
    },
  };
}

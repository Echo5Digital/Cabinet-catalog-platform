...
// Linear hierarchy for the "full access" role ladder. `admin` is intentionally
// excluded — it is a restricted role scoped to specific sections (see
// RESTRICTED_ADMIN_SECTIONS / hasSectionAccess) rather than a rung on this ladder.
const ROLE_LEVELS = { viewer: 0, editor: 1, manager: 2, owner: 3, super_admin: 4 };

// Sections the restricted "admin" role is allowed to access.
const RESTRICTED_ADMIN_SECTIONS = new Set(["leads", "design", "planner"]);
...
export async function getAuthContext(request) {
  ...
  if (serviceKey) {
    ...
    if (entry) {
      ...
      return { user: { id: "service", service: true }, tenantId, role: "manager" };
    }
...
/** Returns true if context.role meets or exceeds minRole. Does not grant access to the restricted "admin" role — use hasSectionAccess for routes under Leads/Design/Planner. */
...
/**
 * Returns true if context can access a given section's routes at minRole.
 * The restricted "admin" role is scoped to leads/design/planner sections only
 * (at any minRole, since it isn't part of the linear ROLE_LEVELS ladder); every
 * other role falls back to the normal hasRole ladder check.
 */
export function hasSectionAccess(context, section, minRole) {
  if (!context.role) return false;
  if (context.role === "admin") return RESTRICTED_ADMIN_SECTIONS.has(section);

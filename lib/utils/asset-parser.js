...
export function parseAssetFilename(filename) {
  ...
  if (parts[0] === "finish") {
    ...
    result.confidence =
      result.lineSlug && result.finishCode ? "partially_matched" : "unmatched";
  ...
  if (skuIndex !== -1) {
    ...
    if (result.lineSlug && result.categorySlug) {
      result.confidence = "partially_matched";
      // Upgrades to 'mapped' after exact SKU DB match in scorer
...
export function scoreAssetConfidence(parsed, dbContext) {
  ...
  if (parsed.assetType === "product") {
  ...
  } else if (parsed.assetType === "finish") {
    ...
    if (lineExists) {
      ...
      result.confidence = parsed.lineSlug ? "partially_matched" : "unmatched";

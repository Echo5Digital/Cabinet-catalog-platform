...
// Handles: B24, SP09, W3630, DB18, PC308424, BL36B, EW1218, OBD12, 308424
const SKU_PATTERN = /^([A-Z]{1,4}\d{2,8}[A-Z]{0,2}|\d{4,})$/i;
  ...
  if (parsed.assetType === "product_diagram") {
    ...
    if (finishes.length > 0) {
      ...
      if (finishMatch) {
        ...
        notes.push(`Auto-matched finish by filename: "${finishMatch.name || finishMatch.code}".`);
        // Category is not required for finish swatches — strip any category notes
        result.parseNotes = notes.filter((n) => !n.toLowerCase().includes("category"));
        return result;
      }
    }

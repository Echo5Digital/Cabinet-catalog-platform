...
export function scoreAssetConfidence(parsed, dbContext) {
  ...
  if (parsed.assetType === "product_diagram") {
    ...
    const colorRecord = parsed.colorCode
      ? colors.find((c) => c.code === parsed.colorCode)
      : null;
    if (colorRecord) {
      result.confidence = "matched";
      ...
      if (parsed.colorCode) notes.push(`Color code "${parsed.colorCode}" not found in DB.`);
    }
  } else if (parsed.assetType === "structure_image") {
    const structRecord = parsed.structureCode
      ? structures.find((s) => s.code === parsed.structureCode)
      : null;
    if (structRecord) {
      result.confidence = "matched";
      ...
      if (parsed.structureCode) notes.push(`Structure code "${parsed.structureCode}" not found in DB.`);
    }
